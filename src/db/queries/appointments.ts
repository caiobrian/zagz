...
export const appointmentsQueries = {
  ...
  update(id: number, fields: Partial<Pick<Appointment, 'status' | 'scheduled_at' | 'notes' | 'provider_phone' | 'provider_address'>>): void {
  ...
  cancel(id: number): void {
    db.prepare(`UPDATE appointments SET status = 'cancelled', updated_at = CURRENT_TIMESTAMP WHERE id = ?`).run(id);
  },
  ...
  markReminder24hSent(id: number): void {
    db.prepare(`UPDATE appointments SET reminder_24h_sent = 1, updated_at = CURRENT_TIMESTAMP WHERE id = ?`).run(id);
  },

  markReminder1hSent(id: number): void {
    db.prepare(`UPDATE appointments SET reminder_1h_sent = 1, updated_at = CURRENT_TIMESTAMP WHERE id = ?`).run(id);
  },
};
