import { appointmentsQueries, type AppointmentStatus } from '../db/queries/appointments.js';
...
export const createAppointmentTool = {
  ...
  description: 'Cria um novo agendamento de serviço (corte de cabelo, consulta médica, lava-jato, manicure, dentista, etc.).',
  parameters: {
    type: 'OBJECT',
    properties: {
      service_type: {
        type: 'STRING',
        description: 'Tipo de serviço (ex: corte_cabelo, consulta_medica, lava_jato, manicure, dentista, consulta_veterinaria)',
      },
      provider_name: {
        type: 'STRING',
      ...
      scheduled_at: {
        type: 'STRING',
        description: 'Data e hora do agendamento no formato ISO 8601 local (ex: 2025-06-15T14:00:00)',
      },
  ...
  parameters: {
    ...
    properties: {
      status: {
        ...
        description: 'Filtrar por status (opcional). Se omitido, lista apenas pendentes/confirmados.',
      },
...
export const updateAppointmentTool = {
  ...
  execute(args: {
    ...
    appointmentsQueries.update(id, toUpdate as any);
