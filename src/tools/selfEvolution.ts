...
export const selfEvolutionTool = {
  name: "evolve_agent",
  description: "Instala um novo servidor MCP ou CLI para expandir as habilidades do agente. Use quando o usuário pedir algo que exija uma ferramenta que você ainda não tem.",
  parameters: {
    type: "OBJECT",
    properties: {
      serverName: {
      ...
      npmPackage: {
        type: "STRING",
        description: "O pacote NPM ou comando npx para rodar o servidor (ex: '@modelcontextprotocol/server-gmail')"
      },
      command: {
        type: "STRING",
  ...
  execute: async (args: any) => {
    ...
    "${args.serverName}": {
      ...
      return `Falha ao evoluir: ${error.message}`;
    }
  }
};
