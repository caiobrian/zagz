...
export const sessionsQueries = {
  ...
  update(id: string, fields: Partial<Pick<Session, 'state' | 'flow'> & { context: object | null }>): void {
    const updates: string[] = ['updated_at = CURRENT_TIMESTAMP'];
    const values: unknown[] = [];

    if (fields.state !== undefined) { updates.push('state = ?'); values.push(fields.state); }
    if (fields.flow !== undefined) { updates.push('flow = ?'); values.push(fields.flow); }
  ...
  complete(id: string): void {
    db.prepare(`UPDATE sessions SET state = 'completed', updated_at = CURRENT_TIMESTAMP WHERE id = ?`).run(id);
  },

  fail(id: string): void {
    db.prepare(`UPDATE sessions SET state = 'failed', updated_at = CURRENT_TIMESTAMP WHERE id = ?`).run(id);
  },
};
