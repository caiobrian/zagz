...
export const memoryReadTool = {
  ...
  description: 'Lê fatos específicos sobre o usuário da memória semântica. Use para consultar preferências, rotinas, dados financeiros ou outros fatos persistentes.',
  parameters: {
    type: 'OBJECT',
    properties: {
      key: {
        type: 'STRING',
        description: 'Chave específica a buscar (ex: "finance.card_avg", "preferences.cinema"). Se omitida, retorna todas as memórias.',
      },
      category: {
        type: 'STRING',
        description: 'Categoria para filtrar (ex: "finance", "projects", "preferences", "routine"). Opcional.',
      },
    },
  },
...
export const memoryWriteTool = {
  ...
  description: 'Salva um fato sobre o usuário na memória semântica para uso futuro. Use para armazenar preferências, localização, dados financeiros, rotinas ou qualquer informação relevante.',
  parameters: {
    type: 'OBJECT',
    properties: {
      key: {
        type: 'STRING',
        description: 'Chave única para o fato (ex: "finance.card_avg", "preferences.cinema", "routine.wake_up", "location.home").',
      },
      value: {
        type: 'STRING',
      ...
      category: {
        type: 'STRING',
        description: 'Categoria do fato: "finance", "projects", "preferences", "routine", "location" ou outra.',
      },
    },
    required: ['key', 'value'],
  },
...
export const cronManageTool = {
  ...
  description: 'Cria, edita, ativa/desativa ou remove rotinas agendadas. Use para gerenciar tarefas automáticas como briefings matinais, alertas financeiros e resumos semanais.',
  parameters: {
    type: 'OBJECT',
    properties: {
      action: {
        type: 'STRING',
        description: 'Ação: "list" (listar jobs), "enable" (ativar), "disable" (desativar), "delete" (remover).',
      },
      name: {
        type: 'STRING',
  ...
  execute(args: { action: string; name?: string; sessionId?: string }): string {
    ...
    switch (args.action) {
      case 'list': {
        ...
        return jobs
          .map(j => `${j.enabled ? '✅' : '❌'} ${j.name} | ${j.schedule} | último: ${j.last_run_at ?? 'nunca'} | status: ${j.last_status ?? '-'}`)
          .join('\n');
      }
      case 'enable': {
